...
class DATABASE(object):
    ...
    def connect(self):
        ...
        try:
            ...
            logger.error("Failed to establish a new connection with InfluxDB, Please check your URL/hostname. Error: {}".format(e))
            time.sleep(120)
    ...
    def query(self, query, meas, Id=False):
        try:
            ...
            if not result:
                ...
                self.data = pd.DataFrame([record.values for table in result for record in table.records])
        
        except (RequestException, ConnectionError) as e:
            logger.error("Failed to connect to InfluxDB. Error: {}".format(e))
    ...
    def cells(self, meas='CellReports', limit=100):
        meas = self.cellmeas
        query = f'from(bucket:"{self.dbname}") |> range(start: -1d) |> filter(fn: (r) => r._measurement == "{meas}") |> limit(n: {limit})'
    ...
    def write_prediction(self, df, meas_name='QP'):
        try:
            ...
            write_api.write(bucket=self.dbname, org=self.org, record=df, data_frame_measurement_name=meas_name)
        except (RequestException, ConnectionError) as e:
            logger.error("Failed to send metrics to InfluxDB. Error: {}".format(e))
